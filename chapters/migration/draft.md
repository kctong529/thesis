...
Connection identifiers help with this deployment problem. A load balancer can use information in the connection identifier to route packets to the backend associated with the connection. Therefore, routing does not need to depend solely on the current address pair. This allows a connection to survive a path change without losing access to the server-side state.

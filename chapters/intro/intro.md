...
Because the network does not track active sessions, the endpoints must determine how each incoming packet relates to stored communication state. In the classical Internet architecture, this association is derived from the source and destination IP addresses together with their port numbers. A transport connection is therefore identified by this tuple. Session identity is implicitly bound to network location.
...
QUIC challenges this assumption by introducing an explicit connection identifier carried within the protocol itself [@rfc9000]. Unlike the traditional 5-tuple, this identifier is independent of IP addressing and can remain stable even as network-layer addresses change. Packets can therefore be associated with existing transport state despite changes in network attachment. This design shifts session identity from an implicit property of routing information to an explicit element of the transport protocol.
